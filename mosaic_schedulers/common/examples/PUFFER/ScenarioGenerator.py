...
def create_scenario(num_puffers=None, base_station=None,science_zone_probability=0.3,plot_scenario=False,science_samples_per_puffer=None):
    '''
    ...
    for puffer in puffer_names:
        ...
        AgentStates[puffer] = {"in_science_zone": in_science_zone, "samples": science_samples[puffer]}
    ...
    # Agents' locations
    for agent in Agents:
        agent_locations[agent] = [random.random()*region_size, random.random()*region_size]
    ...
    for agent1 in Agents:
        for agent2 in Agents:
            if agent1==agent2:
                ...
                bw = compute_bandwidth(agent_locations[agent1], agent_locations[agent2],mode='table')
    ...
    if plot_scenario:
        ...
        for link in CommunicationNetwork:
            ax.plot(
                [agent_locations[link['origin']][0], agent_locations[link['destination']][0]],
                [agent_locations[link['origin']][1], agent_locations[link['destination']][1]],
        ...
        for agent in agent_locations:
            if agent == 'base_station':
                ...
                if AgentStates[agent]["in_science_zone"]:
                    ...
                    markertype = '.k'
                    marker_size = 10
            ax.plot(agent_locations[agent][0], agent_locations[agent][1], markertype, markersize=marker_size)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generates a scenario parameters file for the PUFFER FY18 demo MOSAIC scenario.')
    parser.add_argument('--num_puffers', default=None, help="The number of PUFFERs in the scenario")
    parser.add_argument('--base_station_present', action='store_true', help='Whether a base station is present')
    parser.add_argument('--science_zone_probability', default=0.3, help="The likelihood that a PUFFER is in a science zone")
    parser.add_argument('--random_seed', default=None, help="The random seed for the scenario")
    parser.add_argument('--Plot', action='store_true', help='Plot the scenario and the schedule')
