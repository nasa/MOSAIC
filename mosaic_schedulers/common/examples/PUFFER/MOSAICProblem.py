...
class MOSAICProblem:
    ...
    def partition(self, mode='Bandwidth'):
        ...
        if mode == 'Latency':
            for key, val in self.problemState['link_state'].iteritems():
                ...
                node1 = key[1]
                _val = val.copy()
                _val['length'] = np.linalg.norm(np.array(AgentsDict[node0]['position']) - np.array(AgentsDict[node1]['position']))
                _val['LoS latency'] = _val['length'] / 299792458.
                Links_list.append((node0, node1, _val))
            G.add_edges_from(Links_list)
            # Partition the network
            self.ConnectedComponents = NetworkPartitioning.partition_by_latency(G, max_diameter=self.maxLatency, distance_label='LoS latency')
        elif mode == 'Bandwidth':
            for key, val in self.problemState['link_state'].iteritems():
                node0 = key[0]
                node1 = key[1]
        ...
        NetworkPartitioning.label_connected_components(G, self.ConnectedComponents, plot=False, pos=G.nodes('position_2d'))
        self.G = G
    ...
    def buildScheduler(self):
        ...
        InfeasibleTime = 2 * Thor_s    # A task that can't be done by an agent is set to last this long on the agent
            ...
            for Agent1 in AgentNames:
                ...
                for Agent2 in AgentNames:
                    if (Agent1, Agent2) in problemState['link_state'].keys():
                        CommWindowsBandwidth[t][Agent1][Agent2] = problemState['link_state'][(Agent1, Agent2)]['bandwidth']
                    else:
                        CommWindowsBandwidth[t][Agent1][Agent2] = 0

        # Extract the science availability
        achievable_science_in_horizon = 3  # TODO How many samples can you take within the given horizon

        pufferScienceAvailability = {}
        for puffer in PufferNames:
            in_science_zone = problemState['nodes'][puffer]['in_science_zone']
            pufferScienceAvailability[puffer] = achievable_science_in_horizon * in_science_zone
        ...
        BaseComputationTimeP_housekeeping = {k: int(np.ceil(float(v) / float(TimeStep))) for k, v in BaseComputationTimeP_housekeeping.iteritems()}
        BaseComputationTimeP_science = {k: int(np.ceil(float(v) / float(TimeStep))) for k, v in BaseComputationTimeP_science.iteritems()}
        PufferComputationTimeP_self_housekeeping = {k: int(np.ceil(float(v) / float(TimeStep))) for k, v in PufferComputationTimeP_self_housekeeping.iteritems()}
        PufferComputationTimeP_self_science = {k: int(np.ceil(float(v) / float(TimeStep))) for k, v in PufferComputationTimeP_self_science.iteritems()}
        PufferComputationTimeP_other_housekeeping = {k: int(np.ceil(float(v) / float(TimeStep))) for k, v in PufferComputationTimeP_other_housekeeping.iteritems()}
        PufferComputationTimeP_other_science = {k: int(np.ceil(float(v) / float(TimeStep))) for k, v in PufferComputationTimeP_other_science.iteritems()}
        ...
        for pufferName in PufferNames:
            # Add tasks
            Temp_PufferOptionalTask = {'{}:'.format(pufferName) + k: v for k, v in PufferOptionalTask_housekeeping.iteritems()}
            Temp_PufferTaskReward = {'{}:'.format(pufferName) + k: v for k, v in PufferTaskReward_housekeeping.iteritems()}
            Temp_PufferProductSizes = {'{}:'.format(pufferName) + k: v for k, v in PufferProductSizes_housekeeping.iteritems()}

            for scienceNo in range(pufferScienceAvailability[pufferName]):
                Temp_PufferOptionalTask.update({'{}:{}'.format(pufferName, scienceNo) + k: v for k, v in PufferOptionalTask_science.iteritems()})

                #if pufferScienceReward is None:
                Temp_PufferTaskReward.update({'{}:{}'.format(pufferName, scienceNo) + k: v for k, v in PufferTaskReward_science.iteritems()})
                #else:
                ...
                Temp_PufferProductSizes.update({'{}:{}'.format(pufferName, scienceNo) + k: v for k, v in PufferProductSizes_science.iteritems()})
        ...
        for pufferName in PufferNames:
            for task, tasktime in PufferComputationTimeP_self_housekeeping.iteritems():
                ...
                AllComputationLoad.update({'{}:'.format(pufferName) + task: {pufferName: 1.}})
            ...
            if BaseStationName in AgentNames:  # If the base station is in the group
                for task, tasktime in BaseComputationTimeP_housekeeping.iteritems():
                    ...
                    AllComputationLoad['{}:'.format(pufferName) + task][BaseStationName] = 1.
            ...
            for otherPuffer in PufferNames:
                if otherPuffer != pufferName:
                    for task, tasktime in PufferComputationTimeP_other_housekeeping.iteritems():
                        ...
                        AllComputationLoad['{}:'.format(pufferName) + task][otherPuffer] = 1.
            ...
            for scienceNo in range(pufferScienceAvailability[pufferName]):
                for task, tasktime in PufferComputationTimeP_self_science.iteritems():
                    ...
                    AllComputationLoad.update({'{}:{}'.format(pufferName, scienceNo) + task: {pufferName: 1.}})
                ...
                if BaseStationName in AgentNames:  # If the base station is in the group
                    for task, tasktime in BaseComputationTimeP_science.iteritems():
                        ...
                        AllComputationLoad['{}:{}'.format(pufferName, scienceNo) + task][BaseStationName] = 1.
                ...
                for otherPuffer in PufferNames:
                    if otherPuffer != pufferName:
                        for task, tasktime in PufferComputationTimeP_other_science.iteritems():
                            ...
                            AllComputationLoad['{}:{}'.format(pufferName, scienceNo) + task][otherPuffer] = 1.
        ...
        for pufferName in PufferNames:
            TempPufferHSVColor = np.array([float(PufferNo) / float(N_puffers), 0., .85])
            PufferTaskCounter = 0

            NumPufferTasks = len(PufferTaskList_housekeeping + PufferTaskList_science)
            for PufferTask in PufferTaskList_housekeeping:
                TempTaskHSVColor = np.array([0., .2 + .8 * (1. - float(PufferTaskCounter) / float(NumPufferTasks - 1)), 0.])
                AllTaskColors['{}:'.format(pufferName) + PufferTask] = plt_colors.hsv_to_rgb(TempPufferHSVColor + TempTaskHSVColor)
                PufferTaskCounter += 1
            for PufferTask in PufferTaskList_science:
                TempTaskHSVColor = np.array([0., .2 + .8 * (1. - float(PufferTaskCounter) / float(NumPufferTasks - 1)), 0.])
                for scienceNo in range(pufferScienceAvailability[pufferName]):
                    AllTaskColors['{}:{}'.format(pufferName, scienceNo) + PufferTask] = plt_colors.hsv_to_rgb(TempPufferHSVColor + TempTaskHSVColor)
                PufferTaskCounter += 1
        ...
        # self.FeasibleSolution = [MIP_vars, MIP_vals]
        if CPLEX_Available:
            FeasibleStartDict = self._createFeasibleSolution(Tasks, AgentCapabilities, CommunicationNetwork.CommWindowsBandwidth, PufferNames, PufferComputationTimeP_self_housekeeping, Thor)
            MIP_vars = []
            MIP_vals = []
        ...
        if CPLEX_Available:
            self.scheduler = MOSAICSolver.MOSAICCPLEXScheduler(Thor, AgentCapabilities, Tasks, CommunicationNetwork, self.TimeStep, Options, Verbose=False)
            ...
            self.scheduler = MOSAICSolver.MOSAICGLPKScheduler(Thor, AgentCapabilities, Tasks, CommunicationNetwork, self.TimeStep, Options, Verbose=False)
                ...
                for m in TasksList:
                    for t in range(Thor):
                        if CommWindowsBandwidth[t][i][j] > 0:
                            ...
                            FeasibleStart['C[{},{},{},{}]'.format(i, j, m, t)] = 0.

        for pufferName in PufferNames:
            actionTime = 0
            for task in HousekeepingTasks:
                task_key = "X[{0},{0}:{1},{2}]".format(pufferName, task, actionTime)
                assert task_key in FeasibleStart.keys()
                FeasibleStart[task_key] = 1.
                actionTime += PufferComputationTimeP_self_housekeeping[task]
                for knowledgeTime in range(actionTime, Thor):
                    know_key = "D[{0},{0}:{1},{2}]".format(pufferName, task, knowledgeTime)
    ...
    def schedule(self):
        ...
        if remaining_solver_time < 0:
            ...
            return
        self.scheduler.setTimeLimits(ClockTimeLimit=remaining_solver_time, DetTicksLimit=self.solverDetTicksLimit)
    ...
    def getTaskColors(self):
        if self.isSetUp is False:
            raise Exception("Please set up the problem [problem.setUp()] before calling getTaskColors!")
...
def stringifyState(problemState):
    ...
    for key, val in problemState['link_state'].iteritems():
        newKey = key[0] + ':' + key[1]  # Translate to string. Better hope names do not contain semicolons...
...
def destringifyState(stringState):
    ...
    for key, val in stringState['link_state'].iteritems():
        linkEnds = key.split(':')
        assert len(linkEnds) == 2, 'ERROR: could not convert keys in problemState to strings. Do the agent names contain colons?'
        linkHead = linkEnds[0]
        linkTail = linkEnds[1]
...
if __name__ == "__main__":
    ...
    MProblem = MOSAICProblem(problemState,
                             maxLatency=1e-7,   # Disconnect agents if latency is higher than this value and partition mode is 'Latency'
                             minBandwidth=6,    # Disconnect agents if bandwidth is lower than this value and partition mode is 'Bandwidth'
                             solverClockLimit=60,  # Emergency stop: stop optimization if it takes longer than this many seconds (non-deterministic)
                             solverDetTicksLimit=10000,  # Deterministic time limit: Stop optimization if it takes longer than this many ticks
                             solverParameters=solverParameters,  # Parameters for the MIP solver
                             )

    # Partition the network
    ConnectedComponents = MProblem.partition(mode='Bandwidth')  # Partition the network
    # Build the scheduler
