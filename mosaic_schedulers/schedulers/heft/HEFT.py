...
The key modifications with respect to the standard HEFT scheduler are:
...
class Scheduler():
    def __init__(self,
        ...
        tasks_list = optional_tasks.keys()
        if len(tasks_list) == 0:
        ...
        self.agents_list = self.computation_time[tasks_list[0]].keys()
        ...
        all_tasks = set(self.allowed_agents.keys() + dependency_list.keys())
    ...
    def jsonify_schedule(self, tasks_by_agent):
        json_schedule = []
        for agent, tasks_list in tasks_by_agent.iteritems():
            for task in tasks_list:
                # for task_name, task in task_schedule.iteritems():
                task_dict = {
                    "duration": task["finish_time"] - task["start_time"],
