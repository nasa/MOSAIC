...
class JSONSolver:
    def __init__(self,
        ...
        if 'BandwidthMode' not in JSONInput.keys():
            bandwidth_mode = 'time_varying'

        # Communication bandwidth
        if bandwidth_mode == 'time_varying':
            CommNetwork = self._compute_time_varying_bandwidth(JSONInput,Thor_s,TimeStep)
        elif bandwidth_mode == 'time_invariant':
            CommNetwork = self._compute_time_invariant_bandwidth(JSONInput,Thor_s,TimeStep)
        else:
            print "Bandwidth mode not recognized!"
    ...
    def _compute_time_invariant_bandwidth(self,JSONInput,Thor_s,TimeStep):
        Thor = int(float(Thor_s) / float(TimeStep))
        # Communication bandwidth
        ...
        assert len(set(od_pairs)) == len(od_pairs), "ERROR: some links are repeated (is the problem time-varying?)"
        ...
        for t in range(Thor):
            ...
            # Fill in
            for link in TVBandwidth:
                CommWindowsBandwidth[t][link["origin"]][link["destination"]] = link["bandwidth"]*float(TimeStep)
                CommEnergyCost[t][link["origin"]][link["destination"]] = link["energy_cost"]
    ...
    def _compute_time_varying_bandwidth(self,JSONInput,time_horizon, time_step):
        AgentNames = JSONInput["AgentCapabilities"]["MaxComputationLoad"].keys()
        TVBandwidth = JSONInput["CommunicationNetwork"]
        ...
        # times = list(CommWindows.keys())
        times = np.arange(0,time_horizon,time_step)
        assert len(times) == Thor
        #times.sort()
        ...
        for link in TVBandwidth:
            if link['bandwidth']>0 and link['time_start']<np.max(times) and link['time_end']>np.min(times):
                ...
                link_start_time = np.max([link['time_start'],np.min(times)])
                ...
                # Find the time step immediately before the start of the contact
                just_after_start_time = np.argmin(np.abs(times-link['time_start']))
                if times[just_after_start_time]<=link['time_start'] and just_after_start_time<len(times)-1:
                    just_after_start_time += 1
                
                # How much of this link is inside the block immediately before just_after_start_time?
                first_block_length = np.min([times[just_after_start_time],link_end_time])-link_start_time
                assert just_after_start_time>0
                duration_in_interval = times[just_after_start_time]-times[just_after_start_time-1]
                #fraction_in_interval = float(first_block_length)/float(time_interval_duration)
                #ADD TO BW 
                
                CommWindows[just_after_start_time-1][link_origin][link_destination] += link_bandwidth*duration_in_interval
                CommEnergyCost[just_after_start_time-1][link_origin][link_destination] += link_energy_cost        
                
                for t_ix in range(just_after_start_time, len(times)):
                    t = times[t_ix]
                    if t>link_end_time:
                        break
                    if t_ix >= len(times)-1:  # We cheat at the end
                        time_interval_duration = times[t_ix]-times[t_ix-1]
                    else:
                        time_interval_duration = times[t_ix+1]-times[t_ix]
                    duration_in_interval = np.min([link_end_time,t+time_interval_duration])-t
                    #fraction_in_interval = float(duration_in_interval)/float(time_interval_duration)
                    #ADD TO BW
                    CommWindows[t_ix][link_origin][link_destination] += link_bandwidth*duration_in_interval
...
class MOSAICMILPScheduler:
    ...
    def _ComputeCumulativeCommBandwidth(self):
        ...
        AgentNames = list(self.AgentCapabilities.ComputationTime[TaskNames[0]].keys())
        ...
        for t in range(1, self.Thor + 1):
            ...
            for Agent1 in AgentNames:
                self.CumulativeCommBandwidth[t][Agent1] = {}
                for Agent2 in AgentNames:
                    self.CumulativeCommBandwidth[t][Agent1][Agent2] = self.CommWindowsBandwidth[t - 1][Agent1][Agent2] + self.CumulativeCommBandwidth[t - 1][Agent1][Agent2]
    ...
    def formatToJSON(self):
        ...
        # Create timeline
        Timeline = pd.DataFrame(columns=['Agent', 'Start time', 'Duration', 'Task type', 'Task'])
        # Add tasks
        for taskid in TasksList:
            if (TaskAgent[taskid] is not None) and (TaskSchedule[taskid] is not None):
                ...
                _name = taskid
                Timeline = Timeline.append({'Agent': _agent, 'Start time': _time, 'Duration': _duration, 'Task type': 'Compute', 'Task': _name}, ignore_index=True)
        ...
        for comm in CommSchedule:
            ...
            CommTask = {'Transmitter': _agent, 'Receiver': _receiver, 'Product': _taskname}

            Timeline = Timeline.append({'Agent': _agent, 'Start time': _time, 'Duration': _duration, 'Task type': 'Communicate', 'Task': CommTask}, ignore_index=True)
            Timeline = Timeline.append({'Agent': _receiver, 'Start time': _time, 'Duration': _duration, 'Task type': 'Communicate', 'Task': CommTask}, ignore_index=True)

        Timeline.sort_values(by='Start time', inplace=True)
        Timeline.reset_index(inplace=True, drop=True)

        self.Timeline = Timeline.to_json()
...
class MOSAICCPLEXScheduler(MOSAICMILPScheduler):
    ...
    def setUp(self):
        ...
        if "CostFunction" in Options.keys():
            ...
            CostFunction =  {"energy": 0.0, "total_task_reward": 1.0, "total_time": 0.0 }
        ...
        AgentsList = list(AgentCapabilities.ComputationLoad[TasksList[0]].keys())
            ...
            for m in TasksList:
                ...
                for t in range(Thor - AgentCapabilities.ComputationTime[m][i]):
                    if t >= 0:
                        X[i][m][t] = ix
                        ix += 1
                        X_cost_optional.append(Tasks.TaskReward[m] * Tasks.OptionalTasks[m])
                        X_cost_energy.append(-AgentCapabilities.EnergyCost[m][i])
                    ...
                    for t in range(Thor):
                        if CommWindowsBandwidth[t][i][j] > 0:
                            ...
                            if Options['Packet Comms'] is True:
                                temp_energy_cost = float(self.CommEnergyCost[t][i][j] * CommWindowsBandwidth[t][i][j])
                            else:
                                temp_energy_cost = 0.
                                end_comm_time = self._FindEndCommTime(i, j, m, t)
                                if end_comm_time is None:
                                    end_comm_time = Thor+1
                                for transmission_time in range(t,end_comm_time):
                                    temp_energy_cost += (self.CommEnergyCost[t][i][j] * CommWindowsBandwidth[t][i][j])
        ...
        X_cost = CostFunction["total_task_reward"] * np.array(X_cost_optional,dtype=float) + CostFunction["energy"] * np.array(X_cost_energy,dtype=float)
        C_cost = CostFunction["total_task_reward"] * np.array(C_cost_optional,dtype=float) + CostFunction["energy"] * np.array(C_cost_energy,dtype=float)
        ...
        self.cpx.linear_constraints.add(lin_expr=[cplex.SparsePair([X[i][m][t] for i in AgentsList for t in range(0, max([0, Thor - AgentCapabilities.ComputationTime[m][i]]))],
                                        ...
                                        senses=['G' for m in TasksList if Tasks.OptionalTasks[m] is False],
        ...
        self.cpx.linear_constraints.add(lin_expr=[cplex.SparsePair([X[i][BadTask][t] for i in AgentsList for BadTask in BadPairing for t in range(Thor - AgentCapabilities.ComputationTime[BadTask][i]) if t >= 0],
                                        ...
                                        senses=['L' for BadPairing in Tasks.IncompatibleTasks],
        ...
        self.cpx.linear_constraints.add(lin_expr=TaskDependencyConstraintList,
                                        senses=['L'] * len(TaskDependencyConstraintList),
        ...
        if Options['Packet Comms'] is True:
            ...
            for i in AgentsList:
                for t in range(Thor):
                    VarsList = [X[i][m][tt] for m in TasksList for tt in range(max(0, t - AgentCapabilities.ComputationTime[m][i] + 1),t + 1) if m in X[i].keys() if tt in X[i][m].keys()]
                    CoeffList = [AgentCapabilities.ComputationLoad[m][i] for m in TasksList for tt in range(max(0, t - AgentCapabilities.ComputationTime[m][i] + 1), t + 1) if m in X[i].keys() if tt in X[i][m].keys()]
                    VarsList += [C[i][j][m][t] for m in TasksList for j in AgentsList if CommWindowsBandwidth[t][i][j] > 0 and j != i]
                    CoeffList += [1 for m in TasksList for j in AgentsList if CommWindowsBandwidth[t][i][j] > 0 and j != i]
                    VarsList += [C[j][i][m][t] for m in TasksList for j in AgentsList if CommWindowsBandwidth[t][j][i] > 0 and j != i]
                    CoeffList += [1 for m in TasksList for j in AgentsList if CommWindowsBandwidth[t][j][i] > 0 and j != i]
                    RAConstraintList += [cplex.SparsePair(VarsList, CoeffList)]
            self.cpx.linear_constraints.add(lin_expr=RAConstraintList,
                                            senses=['L'] * len(RAConstraintList),
            ...
            for i in AgentsList:
                for m in TasksList:
                    for t in range(Thor - 1):
                        ...
                        CoeffList = [1, -1]
                        VarsList += [C[j][i][m][tt] for j in AgentsList for tt in range(t + 1) if CommWindowsBandwidth[tt][j][i] > 0]
                        CoeffList += [-np.min([1., np.double(CommWindowsBandwidth[tt][j][i]) / np.double(Tasks.ProductsSize[m])]) for j in AgentsList for tt in range(t + 1) if CommWindowsBandwidth[tt][j][i] > 0]
                        if t - AgentCapabilities.ComputationTime[m][i] + 1 >= 0:
                            VarsList += [X[i][m][t - AgentCapabilities.ComputationTime[m][i] + 1]]
                            CoeffList += [-1]
                        NIConstraintList += [cplex.SparsePair(VarsList, CoeffList)]
            self.cpx.linear_constraints.add(lin_expr=NIConstraintList,
                                            senses=['L'] * len(NIConstraintList),
                                            rhs=[0] * len(NIConstraintList))

        else:
            self._ComputeCumulativeCommBandwidth()

            self._verbprint("Constraint: Resource availability (no packet comms)")
            # Resource availability
            RAConstraintList = []
            for i in AgentsList:
                for t in range(Thor):
                    VarsList = [X[i][m][tt] for m in TasksList for tt in range(max(0, t - AgentCapabilities.ComputationTime[m][i] + 1), t + 1) if tt in X[i][m].keys()]
                    CoeffList = [AgentCapabilities.ComputationLoad[m][i] for m in TasksList for tt in range(max(0, t - AgentCapabilities.ComputationTime[m][i] + 1), t + 1) if tt in X[i][m].keys()]
                    for m in TasksList:
                        for j in AgentsList:
                            MCT_to = self._FindMaxStartCommTime(i, j, m, t)
                            if MCT_to is None:
                                MCT_to = - 1
                            VarsList += [C[i][j][m][tau] for tau in range(MCT_to + 1, t + 1) if CommWindowsBandwidth[tau][i][j] > 0 and j != i]
                            CoeffList += [1 for tau in range(MCT_to + 1, t + 1) if CommWindowsBandwidth[tau][i][j] > 0 and j != i]
                            MCT_from = self._FindMaxStartCommTime(j, i, m, t)
                            if MCT_from is None:
                                MCT_from = -1
                            VarsList += [C[j][i][m][tau] for tau in range(MCT_from + 1, t + 1) if CommWindowsBandwidth[tau][j][i] > 0 and j != i]
                            CoeffList += [1 for tau in range(MCT_from + 1, t + 1) if CommWindowsBandwidth[tau][j][i] > 0 and j != i]
                    RAConstraintList += [cplex.SparsePair(VarsList, CoeffList)]
            self.cpx.linear_constraints.add(lin_expr=RAConstraintList,
                                            senses=['L'] * len(RAConstraintList),
                                            rhs=[1] * len(RAConstraintList))

            self._verbprint("Constraint: New information acquisition (no packet comms)")
            NIConstraintList = []
            # New information acquisition
            for i in AgentsList:
                for m in TasksList:
                    for t in range(Thor - 1):
                        ...
                        CoeffList = [1, -1]
                        for j in AgentsList:
                            MCT = self._FindMaxStartCommTime(j, i, m, t + 1)  # This is the maximum time so that we will have received the information by time t+1
                            if MCT is not None:
                                VarsList += [C[j][i][m][tt] for tt in range(MCT + 1) if CommWindowsBandwidth[tt][j][i] > 0]
                                CoeffList += [-1 for tt in range(MCT + 1) if CommWindowsBandwidth[tt][j][i] > 0]
                        if t - AgentCapabilities.ComputationTime[m][i] + 1 >= 0:
                            VarsList += [X[i][m][t - AgentCapabilities.ComputationTime[m][i] + 1]]
                            CoeffList += [-1]
                        NIConstraintList += [cplex.SparsePair(VarsList, CoeffList)]
        ...
        self.cpx.linear_constraints.add(lin_expr=[cplex.SparsePair([C[i][j][m][t], D[i][m][t]], [1, -1]) for i in AgentsList for j in AgentsList for m in TasksList for t in range(Thor) if CommWindowsBandwidth[t][i][j] > 0],
                                   senses=['L' for i in AgentsList for j in AgentsList for m in TasksList for t in range(Thor) if CommWindowsBandwidth[t][i][j] > 0],
    ...
    def solve(self):
        ...
        self.cpx.solve()
        self._verbprint('Solution status:                   %d' % self.cpx.solution.get_status())
        if self.cpx.solution.is_primal_feasible():
            self.opt_val = self.cpx.solution.get_objective_value()
            self._verbprint('Optimal value:                     {}'.format(self.opt_val))
        ...
        soleps = np.finfo(np.float32).eps  # This is a ugly hack. Sometimes CPLEX will return weird fractional solutions even if we require integral ones. Here, we round.
        for m in self.TasksList:
            ...
            if not FOUND_FLAG:
                TaskSchedule[m] = None
                TaskAgent[m] = None
        TaskAssignment = {'TaskSchedule': TaskSchedule, 'TaskAgent': TaskAgent, 'Cost': self.opt_val}
    ...
    def schedule(self):
        ...
        if solverTimeLimit <= 0:
            return None
        self._verbprint("Setting solver time limit: {}".format(solverTimeLimit))
...
class MOSAICSCIPScheduler(MOSAICMILPScheduler):
    ...
    def setUp(self):
            ...
            for m in TasksList:
                ...
                for t in range(Thor - AgentCapabilities.ComputationTime[m][i]):
                    if t >= 0:
                        ...
                        X_cost[i][m][t] = -CostFunction["total_task_reward"]*float(Tasks.TaskReward[m] * Tasks.OptionalTasks[m]) + CostFunction["energy"]*float(AgentCapabilities.EnergyCost[m][i])
        x_vars_len = ix
                    ...
                    for t in range(Thor):
                        if CommWindowsBandwidth[t][i][j] > 0:
                            ...
                            if Options['Packet Comms'] is True:
                                ...
                                for transmission_time in range(t,end_comm_time):
                                    temp_energy_cost += (self.CommEnergyCost[t][i][j] * CommWindowsBandwidth[t][i][j])
                            C_cost[i][j][m][t] = CostFunction["total_task_reward"]*info_weight + CostFunction["energy"]*float(temp_energy_cost)
                            ix += 1
        ...
        for m in TasksList:
             if Tasks.OptionalTasks[m] is False:
                 self.cpx.addCons(scip.quicksum(X[i][m][t] for i in AgentsList for t in range(0, max([0, Thor - AgentCapabilities.ComputationTime[m][i]])))>= 1.)
                    ...
                    for t in range(Thor - AgentCapabilities.ComputationTime[m][i]):
                        if Tasks.DependencyList[m]:
                            for DepTasks in Tasks.DependencyList[m]:
                                if DepTasks:
                                    self.cpx.addCons(
                                        X[i][m][t] - scip.quicksum(D[i][predecessor][t] for predecessor in DepTasks) <= 0
                                    )
            ...
            for i in AgentsList:
                for t in range(Thor):
                    self.cpx.addCons(
                        ...
                        + scip.quicksum(C[i][j][m][t] for m in TasksList for j in AgentsList if CommWindowsBandwidth[t][i][j] > 0 and j != i)
                        + scip.quicksum(C[j][i][m][t] for m in TasksList for j in AgentsList if CommWindowsBandwidth[t][j][i] > 0 and j != i)
                        <=1
                    )
                ...
                for t in range(Thor):
                    ...
                    for m in TasksList:
                        for j in AgentsList:
                            ...
                            if MCT_to is None:
                                MCT_to = - 1
                            outbound_comms = scip.quicksum(C[i][j][m][tau] for tau in range(MCT_to + 1, t + 1) if CommWindowsBandwidth[tau][i][j] > 0 and j != i)
                            MCT_from = self._FindMaxStartCommTime(j, i, m, t)
                            if MCT_from is None:
                                MCT_from = -1
                            inbound_comms = scip.quicksum(C[j][i][m][tau] for tau in range(MCT_from + 1, t + 1) if CommWindowsBandwidth[tau][j][i] > 0 and j != i)
                    self.cpx.addCons(
                ...
                for m in TasksList:
                    for t in range(Thor - 1):
                        ...
                        for j in AgentsList:
                            MCT = self._FindMaxStartCommTime(j, i, m, t + 1)  # This is the maximum time so that we will have received the information by time t+1
                            if MCT is not None:
                                VarsListInboundInfo += [C[j][i][m][tt] for tt in range(MCT + 1) if CommWindowsBandwidth[tt][j][i] > 0]
                        if t - AgentCapabilities.ComputationTime[m][i] + 1 >= 0:
                            VarsListInboundInfo += [X[i][m][t - AgentCapabilities.ComputationTime[m][i] + 1]]
    ...
    def solve(self):
        ...
        self.cpx.optimize()
        self._verbprint('Solution status:                   {}'.format(self.cpx.getStatus()))
        ...
        soleps = np.finfo(np.float32).eps  # This is a ugly hack. Sometimes the solver will return weird fractional solutions even if we require integral ones. Here, we round.
        for m in self.TasksList:
            FOUND_FLAG = False
            for t in range(self.Thor):
                for i in self.AgentsList:
...
class MOSAICPULPScheduler(MOSAICMILPScheduler):
    ...
    def setUp(self):
        ...
        InfoDiffusionCost = pulp.lpSum(info_weight * C[i][j][m][t] for i in AgentsList for m in TasksList for t in range(Thor) for j in AgentsList)
        # TODO 
            ...
            for j in AgentsList:
                ...
                for m in TasksList:
                    ...
                    for t in range(Thor):
                        if CommWindowsBandwidth[t][i][j]>0:
                            if Options['Packet Comms'] is True:
                                temp_energy_cost[i][j][m][t] = float(self.CommEnergyCost[t][i][j] * CommWindowsBandwidth[t][i][j])
                            else:
                                ...
                                if end_comm_time is None:
                                    end_comm_time = Thor+1
                                for transmission_time in range(t,end_comm_time):
                                    temp_energy_cost[i][j][m][t] += (self.CommEnergyCost[t][i][j] * CommWindowsBandwidth[t][i][j])
        ...
        MILPproblem += CostFunction["total_task_reward"] * (OptionalTaskCost + InfoDiffusionCost) + CostFunction["energy"] * (EnergyCostTask+EnergyCostComm)
        ...
        for m in TasksList:
            if Tasks.OptionalTasks[m] is False:
                MILPproblem += pulp.lpSum(X[i][m][t] for i in AgentsList for t in range(0, max([0, Thor - AgentCapabilities.ComputationTime[m][i]]))) >= 1, "Job {} done".format(m)
            MILPproblem += pulp.lpSum(X[i][m][t] for i in AgentsList for t in range(Thor)) <= 1, "Job {} done once".format(m)

        self._verbprint("Constraint: Incompatible tasks")
        # Incompatible tasks
        IncompatibilityCounter = 0
        for BadPairing in Tasks.IncompatibleTasks:
            MILPproblem += pulp.lpSum(X[i][BadTask][t] for i in AgentsList for BadTask in BadPairing for t in range(Thor - AgentCapabilities.ComputationTime[BadTask][i]) if t >= 0) <= 1, 'Incompatibility %d'%IncompatibilityCounter
                ...
                if m in set(Tasks.DependencyList.keys()):
                    for t in range(Thor):
                        if Tasks.DependencyList[m]:
                            ...
                            for DepTasks in Tasks.DependencyList[m]:
                                if DepTasks:
                                    MILPproblem += X[i][m][t] <= pulp.lpSum(D[i][predecessor][t] for predecessor in DepTasks), "Dependency {0} of task {1} for agent {2} at time {3}".format(DepCounter, m, i, t)
                                    DepCounter += 1
        ...
        if Options['Packet Comms'] is True:
            ...
            for i in AgentsList:
                for m in TasksList:
                    for t in range(Thor - 1):
                        InfoConstraint = D[i][m][t + 1] - D[i][m][t] - pulp.lpSum(C[j][i][m][tt] * np.min([1., np.double(CommWindowsBandwidth[tt][j][i]) / np.double(Tasks.ProductsSize[m])]) for j in AgentsList for tt in range(t + 1)) <= 0
                        if t - AgentCapabilities.ComputationTime[m][i] + 1 >= 0:
                            InfoConstraint -= X[i][m][t - AgentCapabilities.ComputationTime[m][i] + 1]
                        MILPproblem += InfoConstraint, "Information of task {0} at agent {1}, time {2}".format(m, i, t)
        else:
            self._ComputeCumulativeCommBandwidth()
            ...
            for i in AgentsList:
                for t in range(Thor):
                    ResourceAvailabilityConstraintLST = [AgentCapabilities.ComputationLoad[m][i] * X[i][m][tt] for m in TasksList for tt in range(max(0, t - AgentCapabilities.ComputationTime[m][i] + 1), t + 1)]
                    for m in TasksList:
                        for j in AgentsList:
                            ...
                            if MCT_to is None:
                                MCT_to = -1
                            ResourceAvailabilityConstraintLST.append([C[i][j][m][tau] for tau in range(MCT_to + 1, t + 1)])
                            MCT_from = self._FindMaxStartCommTime(j, i, m, t)
                            if MCT_from is None:
                                MCT_from = -1
                            ResourceAvailabilityConstraintLST.append([C[j][i][m][tau] for tau in range(MCT_from + 1, t + 1)])
                    ResourceAvailabilityConstraint = pulp.lpSum(ResourceAvailabilityConstraintLST) <= 1
                    MILPproblem += ResourceAvailabilityConstraint, "Resources {0} at {1}".format(i, t)
                ...
                for m in TasksList:
                    for t in range(Thor - 1):
                        ...
                        for j in AgentsList:
                            MCT = self._FindMaxStartCommTime(j, i, m, t + 1)  # This is the maximum time so that we will have received the information by time t+1
                            if MCT is not None:
                                InfoConstraintLST.append(-pulp.lpSum(C[j][i][m][tt] for tt in range(MCT + 1)))
                        if t - AgentCapabilities.ComputationTime[m][i] + 1 >= 0:
                            InfoConstraintLST.append(-X[i][m][t - AgentCapabilities.ComputationTime[m][i] + 1])
        ...
        for i in AgentsList:
            for j in AgentsList:
                for m in TasksList:
                    for t in range(Thor):
                        MILPproblem += (C[i][j][m][t] <= D[i][m][t]), "Knowledge of task {0} from ag{1} to ag{2} at t{3}".format(m, i, j, t)
        ...
        for i in AgentsList:
            for j in AgentsList:
                for m in TasksList:
                    for t in range(Thor):
                        CommWindowAvailable = int(CommWindowsBandwidth[t][i][j] > 0)
...
class MOSAICGLPKScheduler(MOSAICMILPScheduler):
    ...
    def setUp(self):
        ...
        for i in AgentsList:
            ...
            for m in TasksList:
                ...
                for t in range(Thor):
                    ...
                    ix += 1
                    if t == 0:
                        D_UB.append(int(AgentCapabilities.InitialInformation[m][i]))
                    else:
                        D_UB.append(1)
        ...
        for m in TasksList:
            if Tasks.OptionalTasks[m] is False:
                self.cpx.rows.add(1)
                self.cpx.rows[-1].matrix = [(X[i][m][t], 1.) for i in AgentsList for t in range(0, max([0, Thor - AgentCapabilities.ComputationTime[m][i]]))]
        ...
        for BadPairing in Tasks.IncompatibleTasks:
            self.cpx.rows.add(1)
            self.cpx.rows[-1].matrix = [(X[i][BadTask][t], 1.) for i in AgentsList for BadTask in BadPairing for t in range(Thor - AgentCapabilities.ComputationTime[BadTask][i]) if t >= 0]
            self.cpx.rows[-1].bounds = None, 1.
                    ...
                    for t in range(Thor - AgentCapabilities.ComputationTime[m][i]):
                        if Tasks.DependencyList[m]:
                            for DepTasks in Tasks.DependencyList[m]:
                                if DepTasks:
                                    self.cpx.rows.add(1)
                                    self.cpx.rows[-1].matrix = [(X[i][m][t], 1)] + [(D[i][predecessor][t], -1) for predecessor in DepTasks]
            ...
            for j in AgentsList:
                for m in TasksList:
                    for t in range(Thor):
                        if CommWindowsBandwidth[t][i][j] > 0:
                            self.cpx.rows.add(1)
                            self.cpx.rows[-1].matrix = [(C[i][j][m][t], 1), (D[i][m][t], -1)]
                            self.cpx.rows[-1].bounds = None, 0
    ...
    def solve(self):
        ...
        if retval_lp is None and (self.cpx.status=='opt' or self.cpx.status=='feas'):
            ...
            self._verbprint('Solution status:                   %s' % self.cpx.status)
...
def SchedulePlotter(JSONSchedule, TaskColors):
    Schedule = pd.read_json(JSONSchedule)
    Schedule.sort_index(inplace=True)
    AgentsList = list(set(Schedule['Agent']))
    AgentsList.sort()
    ComputeSchedule = Schedule[Schedule['Task type'] == 'Compute']
    TasksList = list(set(ComputeSchedule['Task']))
    TasksList.sort()
    fig = plt.figure()
    ax = fig.add_subplot(111, aspect='equal')

    # We plot the text in white or black depending on the luminance of the underlying color
    ...
    def PlotScheduledTask(Task, myax):
        AgentID = AgentsList.index(Task['Agent'])
        StartTime = Task['Start time']
        Duration = Task['Duration']
        TaskType = Task['Task']
        if Task['Task type'] == 'Compute':
            # Plot a rectangle for computing tasks
            Rect = patches.Rectangle((StartTime, float(AgentID)), Duration, 1., linewidth=1, edgecolor='k', facecolor=TaskColors[TaskType])
            myax.add_patch(Rect)
            RectCx, RectCy = StartTime + float(Duration) / 2., float(AgentID) + .5
            myax.annotate(TaskType, (RectCx, RectCy), color=BestTextColor(TaskColors[TaskType]), weight='bold', fontsize=6, ha='center', va='center')

        if Task['Task type'] == 'Communicate':
            # Plot two rectangles connected by a line for communication
            CommTask = Task['Task']
            Transmitter = AgentsList.index(CommTask['Transmitter'])
            Receiver = AgentsList.index(CommTask['Receiver'])
            Product = CommTask['Product']
            Rect1 = patches.Rectangle((StartTime, float(Transmitter)), Duration, 1., linewidth=1, edgecolor='k', facecolor=TaskColors[Product])
            myax.add_patch(Rect1)
            Rect1Center = (StartTime + float(Duration) / 2., float(Transmitter) + .5)
            Rect2 = patches.Rectangle((StartTime, float(Receiver)), Duration, 1., linewidth=1, edgecolor='k', facecolor=TaskColors[Product])
            myax.add_patch(Rect2)
            Rect2Center = (StartTime + float(Duration) / 2., float(Receiver) + .5)
            ax.plot([Rect1Center[0], Rect2Center[0]], [Rect1Center[1], Rect2Center[1]], color=TaskColors[Product])

    for _, Task in Schedule.iterrows():
        PlotScheduledTask(Task, ax)

    ax.set_xlim(min(Schedule['Start time']), max(Schedule['Start time'] + Schedule['Duration']))
