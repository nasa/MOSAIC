...
class TestMILPScheduler(unittest.TestCase):
    ...
    def initializeVars(self, N, M, Thor):
        ...
        Options = {}
        Options["CostFunction"] = {"energy": 0.0, "total_task_reward": 1.0, "total_time": 0.0 }
    ...
    def test_independent(self):
        ...
        AgentsList, TasksList, MyAgentCapabilities,MyTasks,CommNetwork,Options = self.initializeVars(self.N,self.M,self.Thor)

        for solver in self.available_solvers():
            Options['Solver'] = solver
            TaskAssignment, CommSchedule, _ = MOSAICSolver.MosaicMILPWrapper(self.Thor,MyAgentCapabilities,MyTasks,CommNetwork,Options)
            self.assertEqual(TaskAssignment['TaskSchedule']['Task 0'], 0)
            self.assertEqual(TaskAssignment['TaskSchedule']['Task 1'], 0)
            self.assertTrue(TaskAssignment['TaskAgent']['Task 0']!=TaskAssignment['TaskAgent']['Task 1'])
    ...
    def test_dependent(self):
        ...
        for solver in self.available_solvers():
            ...
            self.assertEqual(TaskAssignment['TaskAgent']['Task 0'], TaskAssignment['TaskAgent']['Task 1'])
    ...
    def test_or_reqs(self):
        ...
        for solver in self.available_solvers():
            Options['Solver'] = solver
            TaskAssignment, CommSchedule, RawOutput = MOSAICSolver.MosaicMILPWrapper(self.Thor,MyAgentCapabilities,MyTasks,CommNetwork,Options)
            #Tasks are done sequentially
            self.assertEqual(TaskAssignment['TaskSchedule']['Task 0'], 0)
            self.assertEqual(TaskAssignment['TaskSchedule']['Task 1'], 2)
            self.assertEqual(TaskAssignment['TaskSchedule']['Task 2'], None)
    ...
    def test_chain_of_reqs(self):
        ...
        for i in range(self.N):
            MyAgentCapabilities.ComputationTime['Task {}'.format(i)]['Agent {}'.format(i)] = 1

        for m in range(1,len(TasksList)):
            MyTasks.DependencyList[TasksList[m]] = [[TasksList[m-1]]]

        for i in range(self.N-1):
            CommNetwork.CommWindowsBandwidth[2*i+1]['Agent {}'.format(i)]['Agent {}'.format(i+1)] = 1
            CommNetwork.CommEnergyCost[2*i+1]['Agent {}'.format(i)]['Agent {}'.format(i+1)] = 0.1
        ...
        for solver in self.available_solvers():
            ...
            for i in range(self.N):
                self.assertEqual(TaskAssignment['TaskAgent']['Task {}'.format(i)],'Agent {}'.format(i))
                self.assertEqual(TaskAssignment['TaskSchedule']['Task {}'.format(i)],2*i)
    ...
    def test_chain_of_reqs_broken_comms(self):
        ...
        for solver in self.available_solvers():
            ...
            TaskAssignment, CommSchedule, RawOutput = MOSAICSolver.MosaicMILPWrapper(self.Thor,MyAgentCapabilities,MyTasks,CommNetwork,Options)
            for i in range(self.N):
                self.assertEqual(TaskAssignment['TaskAgent']['Task {}'.format(i)],None)
                self.assertEqual(TaskAssignment['TaskSchedule']['Task {}'.format(i)],None )
    ...
    def test_chain_of_reqs_broken_comms_optional_task(self):
        ...
        for solver in self.available_solvers():
            ...
            for i in range(self.M - 1):
                self.assertEqual(TaskAssignment['TaskAgent']['Task {}'.format(i)], 'Agent {}'.format(i))
                self.assertTrue(TaskAssignment['TaskSchedule']['Task {}'.format(i)] >= 2 * i)
    ...
    def test_optional_needed_for_required(self):
        ...
        for solver in self.available_solvers():
            ...
            self.assertTrue((TaskAssignment['TaskSchedule']['Task 1']!=None) or (TaskAssignment['TaskSchedule']['Task 2']!=None))      
    ...
    def test_incompatible_tasks_v3(self):
        ...
        AgentsList, TasksList, MyAgentCapabilities,MyTasks,CommNetwork,Options = self.initializeVars(self.N,self.M,self.Thor)
        MyTasks.OptionalTasks['Task 2'] = True
        MyTasks.IncompatibleTasks = [['Task 0', 'Task 2'], ['Task 1', 'Task 2']]
